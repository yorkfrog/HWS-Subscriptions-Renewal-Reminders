...
class SPRRSendNotifications
{
    ...
    {  
        ...
        foreach($subscription_details as $subscription_detail ) {
			...
			$subscription_period =  $subscription_details->get_billing_period();
			error_log("***** Subs ID / Period:" . $subscription_id . " / " . $subscription_period );
			
			$should_send_for_period = true;
			$should_send_for_period = apply_filters( 'hws_renewal_reminders_should_send_for_period', $should_send_for_period, $subscription_period  );   	
			if ($should_send_for_period) 
			{
            // @@@@@ HWS END
                ...
                if ($user && $options_en=='on') 
                {
                    ...
                    // @@@@@ HWS added $subscription_id param
                    // $body = sprr_renewalremindersemail($subscriber_details['first_name'],$subscriber_details['last_name'],$next_payment_date);
                    $body = sprr_renewalremindersemail($subscriber_details['first_name'],$subscriber_details['last_name'],$next_payment_date,$subscription_id);
